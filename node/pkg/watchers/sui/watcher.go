...
func (w *Watcher) handleObservationRequests(ctx context.Context) error {
	for {
		...
		case r := <-w.obsvReqC:
			...
			if err != nil {
				w.logger.Error("Failed to unmarshal event error message", zap.String("Result", string(body)))
				p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSui, 1)
				return err
			}

			if err_res.Error.Message != nil {
				w.logger.Error("Failed to get events for re-observation request, detected error", zap.String("Result", string(body)))
				p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSui, 1)
